...
import { Box3, Mesh, Texture, Vector3, Plane, PerspectiveCamera } from 'three';
import { GLTFLoader } from 'three/addons/loaders/GLTFLoader.js';
import { MeshoptDecoder } from 'three/addons/libs/meshopt_decoder.module.js';
import { NodeIO } from '@gltf-transform/core';
import { ALL_EXTENSIONS } from '@gltf-transform/extensions';
import { MeshoptDecoder as Decoder } from 'meshoptimizer';
import {
  applyGroups,
  LEVELS,
  PRESETS,
...
const bytes = await fs.readFile('public/villa-v2.glb');
...
const doc = await io.read('public/villa-v2.glb');
await fs.mkdir('work/verified-textures', { recursive: true });
for (const [i, tex] of doc.getRoot().listTextures().entries()) {
  const image = tex.getImage();
  assert(image && image.length > 100);
  await fs.writeFile(
    `work/verified-textures/${i}.${tex.getMimeType() === 'image/png' ? 'png' : 'jpg'}`,
    image,
  );
}
assert(doc.getRoot().listTextures().length >= 18);
assert(
  doc
    .getRoot()
    .listMaterials()
    .some((m) => m.getNormalTexture()),
);
const report = {
  version: '2.0',
