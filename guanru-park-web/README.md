# Guanru Park 3.0 · 建筑漫游

当前交付为 3.0：包含现代室内装修、中西厨设备、两档实时画质、楼层间接光与主卫平面反射，图集提供 15 个视角。完整变更见 [UPGRADE_V3.md](UPGRADE_V3.md)，模型及逐间说明见 [3.0 交付目录](../guanru%20park/展示模型_3.0)。

运行需要 Node.js 22.13.0 或更新版本：
...
npm run build
mkdir -p work
node --experimental-strip-types scripts/verify-v3.mjs
npx tsc --noEmit
```

以下内容保留为 2.0 技术基线说明，当前网页加载的是 `public/assets-v3/manifest.json` 指向的 3.0 资源。
