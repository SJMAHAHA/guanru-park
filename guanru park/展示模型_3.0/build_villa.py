...
def cut(o,points,z,h):
 coords=[o.matrix_basis@v.co for v in o.data.vertices]
 hp=[P(x,y,z) for x,y in points]
 if any(max(v[i] for v in coords)<min(v[i] for v in hp) or min(v[i] for v in coords)>max(v[i] for v in hp) for i in [0,1]):return
...
exec(compile(open(os.path.join(ROOT,'details_v3.py'),encoding='utf-8').read(),'details_v3.py','exec'),globals())
...
bpy.ops.wm.save_as_mainfile(filepath=os.path.join(ROOT,'Guanru_Park_3.0.blend'))
