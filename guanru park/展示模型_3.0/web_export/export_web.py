...
selected_floors=set(sys.argv[sys.argv.index('--')+1:]) if '--' in sys.argv else set()
...
for o in src:
 ...
 if selected_floors and floor not in selected_floors:continue
...
for (floor,cat),objs in groups.items():
 ...
 bpy.ops.object.join()
 o=bpy.context.object;o.name=f'{floor}__{cat}';o['floor']=floor;o['category']=cat;o['version']='3.0'
 out.append({'name':o.name,'floor':floor,'category':cat,'faces':len(o.data.polygons),'vertices':len(o.data.vertices)})
for floor in sorted({k[0] for k in groups}):
 bpy.ops.object.select_all(action='DESELECT')
 for o in list(export_scene.objects):
  if o.get('floor')==floor:o.select_set(True)
 bpy.ops.export_scene.gltf(filepath=str(OUT/(floor+'.glb')),export_format='GLB',use_active_scene=True,use_selection=True,export_apply=False,export_extras=True,export_cameras=False,export_lights=False,export_animations=False,export_texcoords=True,export_normals=True,export_yup=True)
if selected_floors and (OUT/'manifest.json').exists():
 previous=json.loads((OUT/'manifest.json').read_text());out=[r for r in previous if r['floor'] not in selected_floors]+out
(OUT/'manifest.json').write_text(json.dumps(out,indent=2))
print('WEB_EXPORT',len(out),'groups',sum(v['faces'] for v in out),'faces')
